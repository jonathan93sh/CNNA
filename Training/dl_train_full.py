...
def get_callbacks(model_name):
    ...
    checkpoint = ModelCheckpoint(
        ...
        mode='auto',
        period=15,  # checkpoint every epoch #number
        verbose=verbose
    )
    ...
    tb = TensorBoard("./trainv4_tb_logs/"+model_name+"/"+timestamp)
...
def magic(model_name, opt, lrs, fc_layers=(1024, 1024), batch_size=64, dropout=0.5, MAX_EPOCHS=100, dry_run=False, lr=0.0001, acf='relu',pred_acf='softmax',model_from_file=False,
    ...
    opt = opt[1]()
    lrs_name = lrs[0]
    model_name_save = f"{model_name}_detect_full___OPT_{opt_name}___LRS_{lrs_name}_v5_round_no_dropout_opt_calc_with_real"
...
if __name__ == '__main__':
    ...
    def fixAdam(): return Adam_fix(lr=lr, fixW=W_fix_W,
                                   fixI=W_fix_I,autoscale=True)  # ,name='adam_fix')

    def fixSGD(): return SGD_fix(lr=lr, fixW=W_fix_W, fixI=W_fix_I,autoscale=False)#,momentum=0.001*(128/10))
    ...
    #for relufix in relufix_dict:
    #    acf = gen_reluFix(X_fix_W, X_fix_I, relufix[1])
    #    acfObj = Activation(acf,name=relufix[0])
    #    acfObj.__name__ = relufix[0]
    #    get_custom_objects().update({relufix[0] : acfObj})
    ...
    magic(
        ...
        opt=("detect_fixSGD_Wfix_"+str(W_fix_W)+"W_fix_I_"+str(W_fix_I) +
        ...
        dry_run=False,
        acf='relufix',pred_acf='softmaxfix')
