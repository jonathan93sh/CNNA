...
#include <sstream>
//
#include "hwcore/tb/cnn/tb_cnn_2d.hpp"
...
int sc_main(int argc, char *argv[]) {
	...
	// return 0;
	tb_cnn_2d tb_01("tb_CNN");

	cout << "INFO: Simulating " << simulation_name << endl;
	sc_time time_out(250, SC_MS);
	sc_start(time_out);
	// return errors ? 1 : 0;
	cout << "INFO: end time is: " << sc_time_stamp().to_string() << ", and max time is: " << time_out.to_string()
	     << std::endl
	     << std::flush;
	// sc_assert(sc_time_stamp() != time_out); Removed due to simulation error even they are equal???
