...
template <int x, int y, int DIN0, int DIN1, int DIN2, int DIN3, int DIN4, int DIN5, int DOUT>
SC_MODULE(mux_53_128_1_1) {

	sc_in<sc_lv<DIN0> > din0, din1, din2, din3, din4;
	sc_in<sc_lv<DIN5> > din5;
	sc_out<sc_lv<DOUT> > dout;

	void logic() {

		switch (din5.read().to_uint()) {
		case 0:
	...
	SC_CTOR(mux_53_128_1_1) {
		SC_METHOD(logic)
		sensitive << din0 << din1 << din2 << din3 << din4 << din5;
	}
};
